...
$F[U, V](z, x) := \varphi(Uz + Vx)$, $G[W](z) := \Phi(Wz)$, $W \in \R^{q \times m}$
\end{subsubsectionbox}

\begin{subsubsectionbox}{BPTT}
$\frac{\partial h}{\partial z_i^t} = \sum_{s=t}^T \sum_{k=1}^m \sum_j \frac{\partial \hat{y}_k^s}{\partial z_j^s} \frac{\partial z_j^s}{\partial z_i^t}$, $\frac{\partial \hat{y}_k^s}{\partial z_j^s} = \dot{\Phi}_k^s w_{kj}$
...
$z_t = (1 - \sigma) \odot z_{t-1} + \sigma \odot \tilde{z}_t$, $\sigma := \sigma(G[x_t, z_{t-1}])$

$\tilde{z}_t := \tanh(V[\ell_t \odot z_{t-1}, x_t])$, $\ell_t := \sigma(H[z_{t-1}, x_t])$
\end{subsubsectionbox}
...
$\theta_i \sim \text{Uni}[0; 2\pi]$, $r_i \sim \text{Uni}[I]$, $I \subseteq [0; 1]$
\end{subsubsectionbox}
